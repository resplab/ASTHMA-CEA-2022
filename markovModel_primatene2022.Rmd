...
### Model Setup & Parameter Functions ###
...
df_psa <- data.frame(
    ...
    ### Cost and utility inputs ###
      ...
      ## Utility
      du.AU   = du.AU <- -0.07/52,                # disutility of asthma uncontrolled week
...
```{r, Probablistic}
################################# Cost-effectiveness analysis #############################
sim_PSA_no_trt  <- f_wrapper(fxn=f_gen_psa, n_sim=1000)
sim_PSA_PRIM    <- f_wrapper(fxn=f_gen_psa, n_sim=1000, trt = "primatene") 
sim_PSA_SYMB    <- f_wrapper(fxn=f_gen_psa, n_sim=1000, trt="symbicort")  
...
#All results
  ...
  write_csv(res,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\PSAResults_ALLv2.csv")
...
```{r, CEA plotting}
#Importing data
res <- read_csv("C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\PSAResults_ALLv2.csv")
res$ID <- rep(1:1000,3)
...
### Cost-effectiveness Plane ###
  ...
  return(p)
    ...
    CEA_plot(data = resw, x = resw$deltaSP_QALYs, y = resw$deltaSP_Costs, title = "SYMB vs PRIM")
    CEA_plot(data = resw, x = resw$deltaNP_QALYs, y = resw$deltaNP_Costs, title = "noTrt vs PRIM")

#CEAC
  CEAC_plot <- function(costs, qalys, title=NULL){
  ...
  SP <- CEAC_plot(costs=resw$deltaSP_Costs, qalys=resw$deltaSP_QALYs) #SYMB vs PRIM
  NP <- CEAC_plot(costs=resw$deltaNP_Costs, qalys=resw$deltaNP_QALYs) #No Trt vs PRIM
