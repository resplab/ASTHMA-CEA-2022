...
### OTHER ###  
  #H0 entering H1
  if(H0==FALSE){pAnn.H0_H1 <- 0}
  if(H0==TRUE){pAnn.H0_H1 <- c(0,0.2)}
...
### SEV (Severe Exacerbation) ###
  ...
  #Cumulative rate from SYGMA trials
  r.H0_SEVtot <- 0.20 #syg1
  if(icssaba==F){r.SYM_SEVtot <- 0.11} #syg2
  if(icssaba==T){r.SYM_SEVtot <- c(0.1*0.11, 1.5*0.11)}
  if(syg2==F){r.H1_SEVtot <- 0.09} #syg1
  if(syg2==T){r.H1_SEVtot <- c(0.09, 0.12)} #syg1
...
### Cost and utility inputs ###
  ...
  #Med inhalations & Drug Acquisition
    if(prim==FALSE){inh.PRIM <- 0.52}
    if(sym==FALSE){inh.SYM <- 0.52}
    inh.ICS <- 2
    if(saba==FALSE){inh.SABA <- 0.49}
    
    if(prim==T){inh.PRIM <- c(1/7, 1)}
    if(sym==T){inh.SYM <- c(1/7, 1)}
    if(saba==T){inh.SABA <- c(1/7, 1)}
  ...
  if(payer==T){
  c.timeHCP  <- c(0,3.03) # HCP management time/week*Assuming 240 total minutes of 4 PCP visits per year, 20 min travel time, 40min wait, median wage $1037 in 2022
  c.prodAU   <- c(0,132)  # cost of productivity loss due to asthma not controlled days per week*
  
  #Productivity loss due to severe exacerbation ** Assuming do not go to work for 2,4,7 days
  c.prodSCS  <- c(0,296) # productive lose due to severe exacerbation.
  c.prodED <- c(0,593)
  c.prodHosp <- c(0,1037)}
...
```{r, Deterministic OSA}
m <- read_csv("mortality.csv")
mort.list <- rep(list(m),2)

#H0_H1
  sim_PSA_no_trt  <- f_wrapper(n_sim=2, fxn = f_gen_dsa(H0=TRUE))
  sim_PSA_PRIM    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(H0=TRUE), trt = "primatene") 
  sim_PSA_SYMB    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(H0=TRUE), trt="symbicort")
  
  sim_PSA_no_trt$trt <- "noTrt"
  sim_PSA_PRIM$trt <- "PRIM"
  sim_PSA_SYMB$trt <- "SYMB"
  H0_H1 <- rbind(sim_PSA_no_trt, sim_PSA_PRIM, sim_PSA_SYMB) 
  
#prim inhalations
  sim_PSA_no_trt  <- f_wrapper(n_sim=2, fxn = f_gen_dsa(prim=TRUE))
  sim_PSA_PRIM    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(prim=TRUE), trt = "primatene") 
  sim_PSA_SYMB    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(prim=TRUE), trt="symbicort")

  sim_PSA_no_trt$trt <- "noTrt"
  sim_PSA_PRIM$trt <- "PRIM"
  sim_PSA_SYMB$trt <- "SYMB"
  prim_inh <- rbind(sim_PSA_no_trt, sim_PSA_PRIM, sim_PSA_SYMB)  

#saba inhalations
  sim_PSA_no_trt  <- f_wrapper(n_sim=2, fxn = f_gen_dsa(saba=TRUE))
  sim_PSA_PRIM    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(saba=TRUE), trt = "primatene") 
  sim_PSA_SYMB    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(saba=TRUE), trt="symbicort")

  sim_PSA_no_trt$trt <- "noTrt"
  sim_PSA_PRIM$trt <- "PRIM"
  sim_PSA_SYMB$trt <- "SYMB"
  saba_inh <- rbind(sim_PSA_no_trt, sim_PSA_PRIM, sim_PSA_SYMB)  
  
#sym inhalations
  sim_PSA_no_trt  <- f_wrapper(n_sim=2, fxn = f_gen_dsa(sym=TRUE))
  sim_PSA_PRIM    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(sym=TRUE), trt = "primatene") 
  sim_PSA_SYMB    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(sym=TRUE), trt="symbicort")

  sim_PSA_no_trt$trt <- "noTrt"
  sim_PSA_PRIM$trt <- "PRIM"
  sim_PSA_SYMB$trt <- "SYMB"
  sym_inh <- rbind(sim_PSA_no_trt, sim_PSA_PRIM, sim_PSA_SYMB)  
  
#payer perspective
  sim_PSA_no_trt  <- f_wrapper(n_sim=2, fxn = f_gen_dsa(payer=TRUE))
  sim_PSA_PRIM    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(payer=TRUE), trt = "primatene") 
  sim_PSA_SYMB    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(payer=TRUE), trt="symbicort")

  sim_PSA_no_trt$trt <- "noTrt"
  sim_PSA_PRIM$trt <- "PRIM"
  sim_PSA_SYMB$trt <- "SYMB"
  payer <- rbind(sim_PSA_no_trt, sim_PSA_PRIM, sim_PSA_SYMB)  

#sygma 2 exac rate for H1
  sim_PSA_no_trt  <- f_wrapper(n_sim=2, fxn = f_gen_dsa(syg2=TRUE))
  sim_PSA_PRIM    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(syg2=TRUE), trt = "primatene") 
  sim_PSA_SYMB    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(syg2=TRUE), trt="symbicort")
  
  sim_PSA_no_trt$trt <- "noTrt"
  sim_PSA_PRIM$trt <- "PRIM"
  sim_PSA_SYMB$trt <- "SYMB"
  syg2 <- rbind(sim_PSA_no_trt, sim_PSA_PRIM, sim_PSA_SYMB)  

#different exac rates for symbicort
  sim_PSA_no_trt  <- f_wrapper(n_sim=2, fxn = f_gen_dsa(icssaba=TRUE))
  sim_PSA_PRIM    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(icssaba=TRUE), trt = "primatene") 
  sim_PSA_SYMB    <- f_wrapper(n_sim=2, fxn = f_gen_dsa(icssaba=TRUE), trt="symbicort") 

  sim_PSA_no_trt$trt <- "noTrt"
  sim_PSA_PRIM$trt <- "PRIM"
  sim_PSA_SYMB$trt <- "SYMB"
  icssaba <- rbind(sim_PSA_no_trt, sim_PSA_PRIM, sim_PSA_SYMB)  

rm(sim_PSA_no_trt,sim_PSA_PRIM,sim_PSA_SYMB)

list(H0_H1,prim_inh,saba_inh,sym_inh, payer, syg2, icssaba)
#results_wide
