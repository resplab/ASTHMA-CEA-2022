...
### Model input
  ...
  v.n   <- c("H0C","H0NC", #H0 no hcp mgt
             "H1C", "H1NC", #H1 HCP MGT
             "SCS","ED","HOSP", #SEV exacerbation
             "D") #death
  
  n.s   <- length(v.n)                 # the number of health states
  v.M_1 <- c(1,0,0,0,0,0,0,0)          # Initial health state distribution - Everyone starts at controlled no HCP management
...
#Asthma Fatality (baseline)
  ...
  #H0
    p.H0C_H1 <- p.H0NC_H1 <- 0                     # weekly probability entering HCP from no HCP management (can't go to H1 without sev)
    pAnn.H0C_D <- pAnn.H0NC_D   <- mortality$b_mortality   # probability of death from no HCP management (turn into vector)
...
for (i in 2:(n.t + 1)) { 
  ...
  H0C.rem <- min((0+0+p.H0C_SCS+p.H0C_ED+p.H0C_Hosp+p.H0C_D[j]),1)
  H0NC.rem <- min((0+0+p.H0NC_SCS+p.H0NC_ED+p.H0NC_Hosp+p.H0NC_D[j]),1)
  H1C.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1C_SCS+p.H1C_ED+p.H1C_Hosp+p.H1C_D[j]),1)
  H1NC.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1NC_SCS+p.H1NC_ED+p.H1NC_Hosp+p.H1C_D[j]),1)
  SEV.rem <- min((p.SCS_D_PRIM[j]+p.ED_D_PRIM[j]+p.Hosp_D_PRIM[j]+p.H0C_D[j]),1)
    

  # create the transition matrix for each model cycle
    m.P <- matrix(c(
                    (1-H0C.rem)*p.H0_AC,(1-H0C.rem)*(1-p.H0_AC), 0,0, p.H0C_SCS, p.H0C_ED, p.H0C_Hosp, p.H0C_D[j], #H0C
                    (1-H0NC.rem)*p.H0_AC,(1-H0NC.rem)*(1-p.H0_AC), 0,0, p.H0NC_SCS, p.H0NC_ED, p.H0NC_Hosp, p.H0NC_D[j], #H0NC 
                    p.H1_H0*p.H0_AC,p.H1_H0*(1-p.H0_AC),(1 - H1C.rem)*p.H1_AC,(1 - H1C.rem)*(1-p.H1_AC),p.H1C_SCS,p.H1C_ED,p.H1C_Hosp, p.H1C_D[j], #H1C
                    p.H1_H0*p.H0_AC,p.H1_H0*(1-p.H0_AC),(1 - H1NC.rem)*p.H1_AC,(1 - H1NC.rem)*(1-p.H1_AC),p.H1NC_SCS,p.H1NC_ED,p.H1NC_Hosp, p.H1NC_D[j], #H1NC
                    0, 0, (1 - SEV.rem)*p.H1_AC,(1-SEV.rem)*(1-p.H1_AC), 0, 0, 0, min(p.SCS_D_PRIM[j]+p.H0C_D[j],1), #SCS
                    0, 0, (1 - SEV.rem)*p.H1_AC,(1-SEV.rem)*(1-p.H1_AC), 0, 0, 0, min(p.ED_D_PRIM[j]+p.H0C_D[j],1), #ED
                    0, 0, (1 - SEV.rem)*p.H1_AC,(1-SEV.rem)*(1-p.H1_AC), 0, 0, 0, min(p.Hosp_D_PRIM[j]+p.H0C_D[j],1), # Hosp
                    0, 0, 0, 0, 0, 0, 0, 1), #D
  ...
  v.c <- c(
           ...
           c.ICS_SABA + c.HCP + c.timeHCP + c.AU, #H1C
           c.SCS + c.prodSCS, #SCS
           c.ED + c.prodED, #ED
...
H0C.rem <- min((0+0+p.SYMC_SCS+p.SYMC_ED+p.SYMC_Hosp+p.H0C_D[j]),1)
H0NC.rem <- min((0+0+p.SYMNC_SCS+p.SYMNC_ED+p.SYMNC_Hosp+p.H0NC_D[j]),1)
H1C.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1C_SCS+p.H1C_ED+p.H1C_Hosp+p.H1C_D[j]),1)
H1NC.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1NC_SCS+p.H1NC_ED+p.H1NC_Hosp+p.H1C_D[j]),1)
SEV.rem <- min((p.SCS_D[j]+p.ED_D[j]+p.Hosp_D[j]+p.H0C_D[j]),1)
    

  # create the transition matrix for each model cycle
    m.P <- matrix(c(
                    (1-H0C.rem)*p.SYM_AC,(1-H0C.rem)*(1-p.SYM_AC), 0,0, p.SYMC_SCS, p.SYMC_ED, p.SYMC_Hosp, p.H0C_D[j], #H0C
                    (1-H0NC.rem)*p.SYM_AC,(1-H0NC.rem)*(1-p.SYM_AC), 0,0, p.SYMNC_SCS, p.SYMNC_ED, p.SYMNC_Hosp, p.H0NC_D[j], #H0NC 
                    p.H1_H0*p.SYM_AC,p.H1_H0*(1-p.SYM_AC),(1 - H1C.rem)*p.H1_AC,(1 - H1C.rem)*(1-p.H1_AC),p.H1C_SCS,p.H1C_ED,p.H1C_Hosp, p.H1C_D[j], #H1C
                    p.H1_H0*p.SYM_AC,p.H1_H0*(1-p.SYM_AC),(1 - H1NC.rem)*p.H1_AC,(1 - H1NC.rem)*(1-p.H1_AC),p.H1NC_SCS,p.H1NC_ED,p.H1NC_Hosp, p.H1NC_D[j], #H1NC
                    0, 0, (1 - SEV.rem)*p.H1_AC,(1-SEV.rem)*(1-p.H1_AC), 0, 0, 0, min(p.SCS_D[j]+p.H0C_D[j],1), #SCS
                    0, 0, (1 - SEV.rem)*p.H1_AC,(1-SEV.rem)*(1-p.H1_AC), 0, 0, 0, min(p.ED_D[j]+p.H0C_D[j],1), #ED
                    0, 0, (1 - SEV.rem)*p.H1_AC,(1-SEV.rem)*(1-p.H1_AC), 0, 0, 0, min(p.Hosp_D[j]+p.H0C_D[j],1), # Hosp
                    0, 0, 0, 0, 0, 0, 0, 1), #D
                  nrow = n.s, ncol = n.s, byrow = T,
                  dimnames = list(v.n, v.n))
...
delta.C2 <- v.C[2] - v.C[1]            # calculate incremental costs
delta.E2 <- v.E[2] - v.E[1]            # calculate incremental QALYs
ICER2 <- delta.C2 / delta.E2             # calculate the ICER

delta.C3 <- v.C[3] - v.C[1]            # calculate incremental costs
delta.E3 <- v.E[3] - v.E[1]            # calculate incremental QALYs
ICER3 <- delta.C3 / delta.E3             # calculate the ICER

  # results <- c(delta.C, delta.E, ICER)  # store the values in a new variable

# Create full incremental cost-effectiveness analysis table
table_markov <- data.frame(
  round(v.C, 2),              # costs per arm
  round(v.E, 5),              # health outcomes per arm
  c("", round(delta.C2, 2), round(delta.C3,2)),   # incremental costs
  c("", round(delta.E2, 5), round(delta.E3,5)),   # incremental QALYs
  c("", round(ICER2, 2), round(ICER3,2))       # ICER
)
...
table_markov                    # print the table 
 ...
 # write.table(table_markov, file = "CEA_markov_v1.csv", sep = ",", quote = FALSE, row.names = F)
