...
### SEV (Severe Exacerbation) ###
  ...
  if(DSA=="adh.sym"){r.SYM_SEVtot <- (0.11*c(1, 0.8)) + 0.20*c(0,0.2)}
...
### Cost and utility inputs ###
  ...
  #Med inhalations & Drug Acquisition
    ...
    if(DSA=="adh.sym"){inh.SYM <- (0.52*c(1, 0.8)) + 0*c(0, 0.2)}
  ...
  u.H0 <- 0.87/52          # utility of non-exacerbation
  if(DSA=="u.H0"){u.H0 <- c(0.87/52*0.8, 0.87/52*1)}
  u.H1 <- u.H0
  u.SCS <- u.H0 + du.SCS     # utility of SCS
  u.ED  <- u.H0 + du.ED        # utility of emergency deparment 
  u.Hosp<- u.H0 + du.Hosp   # utility of hospitalization
...
dsa_params<-c("H0_H1","H1_H0",
               ...
               "du.AU","du.SCS","du.ED","du.Hosp","u.H0", "adh.sym")
...
############################### Markov Model  ###########################
# The Markov function 
Markov <- function(params, v.M_1, n.t, v.n, d.c, d.e, trt=FALSE, res=F) {
# Arguments:  
...
for (i in 2:(n.t + 1)) { 
  ...
  if(p.H0C_D[i-1] == 1){ #last cycle if background mortality ==1 then set everything to 0
     m.P<-  matrix(c(
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #H0C
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #H0NC
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #H1C
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #H1NC
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #SCS
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #ED
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #Hosp
                  0, 0, 0, 0, 0, 0, 0, 0, 1, #SEVR
                  0, 0, 0, 0, 0, 0, 0, 0, 1),#D
...
write.csv(t,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\BaseResults19.csv")
...
### setting up PSA ###
  ...
  mort.fxn <- function(){
        mortality <- read_csv("mortality19.csv") %>% select(-pSp.SCS_AF,-pSp.ED_AF,-pSp.HOSP_AF)
...
#All results
  ...
  # write_csv(res,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\PSAResults_ALL19.csv")
  # write_csv(psa,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\PSAParams_ALL19.csv")
...
##Discounting##
  ...
  # write_csv(res_dc0,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_dc0.csv")
  ...
  # write_csv(res_dc6,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_dc6.csv")
...
##Time Horizon##
  ...
  # write_csv(res_th10,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_th10.csv")
  ...
  # write_csv(res_th30,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_th30.csv")
...
#Payer Perspective
  ...
  # write_csv(res_public,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_public.csv")
...
dsa_setup <- function(df,title){
  ...
  res <- rbind(sim_base_no_trt,sim_base_PRIM,sim_base_SYMB)
  # write_csv(res,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\DSAResults19.csv")
  
```
```{r, microsimulation}
# Model input
n.i   <- 100000              # number of simulated individuals
n.t   <- 4213                # time horizon
v.n   <- c("H0C","H0NC", #H0 no hcp mgt
               "H1C", "H1NC", #H1 HCP MGT
               "SCS","ED","Hosp", "SEVR", #SEV exacerbation
               "D") #death 
...
MicroSim <- function(v.M_1, n.i, n.t, v.n, d.c, d.e, TR.out = TRUE, TS.out = TRUE, Trt = FALSE, seed = 1996) {
  ...
  mortality <- read_csv("mortality19.csv") #running in mortality sheet
  params <- f_gen_base() #running in parameter sheet
  # list2env(as.list(params), envir=globalenv()) #parameter to environment
...
Probs <- function(M_it) { 
  ...
  m.p.it[,M_it == "H0C"]  <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "H0NC"]  <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "H1C"]  <-c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "H1NC"]  <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "SCS"] <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "ED"] <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "Hosp"] <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "SEVR"] <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  m.p.it[,M_it == "D"]  <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
    
  }else{
  
  # update m.p.it with the appropriate probabilities 
  m.p.it[,M_it == "H0C"]  <- c((1-H0C.rem)*p.H0_AC,(1-H0C.rem)*(1-p.H0_AC), p.H0_H1*p.H1_AC, p.H0_H1*(1-p.H1_AC), p.H0C_SCS, p.H0C_ED, p.H0C_Hosp, 0, p.H0C_D[t-1])
  m.p.it[,M_it == "H0NC"]  <- c((1-H0NC.rem)*p.H0_AC,(1-H0NC.rem)*(1-p.H0_AC), p.H0_H1*p.H1_AC, p.H0_H1*(1-p.H1_AC), p.H0NC_SCS, p.H0NC_ED, p.H0NC_Hosp, 0, p.H0NC_D[t-1])
  m.p.it[,M_it == "H1C"]  <- c(p.H1_H0*p.H0_AC,p.H1_H0*(1-p.H0_AC),(1 - H1C.rem)*p.H1_AC,(1 - H1C.rem)*(1-p.H1_AC),p.H1C_SCS,p.H1C_ED,p.H1C_Hosp,0, p.H1C_D[t-1])
  m.p.it[,M_it == "H1NC"]  <- c(p.H1_H0*p.H0_AC,p.H1_H0*(1-p.H0_AC),(1 - H1NC.rem)*p.H1_AC,(1 - H1NC.rem)*(1-p.H1_AC),p.H1NC_SCS,p.H1NC_ED,p.H1NC_Hosp,0, p.H1NC_D[t-1])
  m.p.it[,M_it == "SCS"] <- c(0, 0, 0, 0, 0, 0, 0,(1-SCS.rem), min(p.SCS_D[t-1]+p.H0C_D[t-1],1))
  m.p.it[,M_it == "ED"] <- c(0, 0, 0, 0, 0, 0, 0,(1-ED.rem), min(p.ED_D[t-1]+p.H0C_D[t-1],1))
  m.p.it[,M_it == "Hosp"] <- c(0, 0, 0, 0, 0, 0, 0,(1-Hosp.rem), min(p.Hosp_D[t-1]+p.H0C_D[t-1],1))
  m.p.it[,M_it == "SEVR"] <- c(0, 0, p.SEVR_H1*p.H1_AC, p.SEVR_H1*(1-p.H1_AC), 0, 0, 0, 1-SEVR.rem, p.H0C_D[t-1])
  m.p.it[,M_it == "D"]  <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  }
  ifelse(round(colSums(m.p.it), digits=10)==1, return(t(m.p.it)), print("Probabilities do not sum to 1")) # return the transition probabilities or produce an error
}       
...
Effs <- function (M_it, cl = 1) {
  ...
  #Remaining health states
  H0C.rem <- min((p.H0_H1*p.H1_AC+p.H0_H1*(1-p.H1_AC)+p.H0C_SCS+p.H0C_ED+p.H0C_Hosp+p.H0C_D[t-1]),1)
  H0NC.rem <- min((p.H0_H1*p.H1_AC+p.H0_H1*(1-p.H1_AC)+p.H0NC_SCS+p.H0NC_ED+p.H0NC_Hosp+p.H0NC_D[t-1]),1)
  H1C.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1C_SCS+p.H1C_ED+p.H1C_Hosp+p.H1C_D[t-1]),1)
  H1NC.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1NC_SCS+p.H1NC_ED+p.H1NC_Hosp+p.H1C_D[t-1]),1)
  
  SCS.rem <- min((p.SCS_D_PRIM[t-1]+p.H0C_D[t-1]),1)
  ED.rem <- min((p.ED_D_PRIM[t-1]+p.H0C_D[t-1]),1)
  Hosp.rem <-min((p.Hosp_D_PRIM[t-1]+p.H0C_D[t-1]),1)
  SEVR.rem <- min((p.SEVR_H1*p.H1_AC + p.SEVR_H1*(1-p.H1_AC)+p.H0C_D[t-1]),1)
  ...
  m.p.it[,M_it == "H1C"]  <- c(p.H1_H0*p.H0_AC,p.H1_H0*(1-p.H0_AC),(1 - H1C.rem)*p.H1_AC,(1 - H1C.rem)*(1-p.H1_AC),p.H1C_SCS,p.H1C_ED,p.H1C_Hosp, 0, p.H1C_D[t-1])
  m.p.it[,M_it == "H1NC"]  <- c(p.H1_H0*p.H0_AC,p.H1_H0*(1-p.H0_AC),(1 - H1NC.rem)*p.H1_AC,(1 - H1NC.rem)*(1-p.H1_AC),p.H1NC_SCS,p.H1NC_ED,p.H1NC_Hosp, 0, p.H1NC_D[t-1])
  m.p.it[,M_it == "SCS"] <- c(0, 0, 0, 0, 0, 0, 0,(1-SCS.rem), min(p.SCS_D_PRIM[t-1]+p.H0C_D[t-1],1))
  m.p.it[,M_it == "ED"] <- c(0, 0, 0, 0, 0, 0, 0,(1-ED.rem), min(p.ED_D_PRIM[t-1]+p.H0C_D[t-1],1))
  m.p.it[,M_it == "Hosp"] <- c(0, 0, 0, 0, 0, 0, 0,(1-Hosp.rem), min(p.Hosp_D_PRIM[t-1]+p.H0C_D[t-1],1))
  m.p.it[,M_it == "SEVR"] <- c(0, 0, p.SEVR_H1*p.H1_AC, p.SEVR_H1*(1-p.H1_AC), 0, 0, 0, 1-SEVR.rem, p.H0C_D[t-1])
  m.p.it[,M_it == "D"]  <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
  }
  ifelse(round(colSums(m.p.it), digits=10)==1, return(t(m.p.it)), print("Probabilities do not sum to 1")) # return the transition probabilities or produce an error
  ...
  Costs <- function (M_it) {
    ...
    c.it <- 0                                  # by default the cost for everyone is zero 
    c.it[M_it == "H0C"]  <- c.PRIM
    c.it[M_it == "H0NC"]  <-  c.PRIM + c.AU + c.prodAU
    c.it[M_it == "H1C"]  <- c.ICS_SABA + c.HCP + c.timeHCP
    c.it[M_it == "H1NC"]  <- c.ICS_SABA + c.HCP + c.timeHCP + c.AU + c.prodAU
    c.it[M_it == "SCS"]  <- c.SCS + c.prodSCS
    c.it[M_it == "ED"]  <-  c.ED + c.prodED
  ...
  Probs <- function(M_it) { 
    ...
    #Remaining health states
    H0C.rem <- min((p.H0_H1*p.H1_AC+p.H0_H1*(1-p.H1_AC)+p.SYMC_SCS+p.SYMC_ED+p.SYMC_Hosp+p.H0C_D[t-1]),1)
    H0NC.rem <- min((p.H0_H1*p.H1_AC+p.H0_H1*(1-p.H1_AC)+p.SYMNC_SCS+p.SYMNC_ED+p.SYMNC_Hosp+p.H0NC_D[t-1]),1)
    H1C.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1C_SCS+p.H1C_ED+p.H1C_Hosp+p.H1C_D[t-1]),1)
    H1NC.rem <- min(((p.H1_H0*p.H0_AC)+(p.H1_H0*(1-p.H0_AC))+p.H1NC_SCS+p.H1NC_ED+p.H1NC_Hosp+p.H1C_D[t-1]),1)
  ...
  m.p.it[,M_it == "D"]  <- c(0, 0, 0, 0, 0, 0, 0, 0, 1)
    ...
    # update m.p.it with the appropriate probabilities 
    m.p.it[,M_it == "H0C"]  <- c((1-H0C.rem)*p.SYM_AC,(1-H0C.rem)*(1-p.SYM_AC), p.H0_H1*p.H1_AC, p.H0_H1*(1-p.H1_AC), p.SYMC_SCS, p.SYMC_ED, p.SYMC_Hosp, 0, p.H0C_D[t-1])
    m.p.it[,M_it == "H0NC"]  <- c((1-H0NC.rem)*p.SYM_AC,(1-H0NC.rem)*(1-p.SYM_AC), p.H0_H1*p.H1_AC, p.H0_H1*(1-p.H1_AC), p.SYMNC_SCS, p.SYMNC_ED, p.SYMNC_Hosp, 0, p.H0NC_D[t-1])
    m.p.it[,M_it == "H1C"]  <- c(p.H1_H0*p.SYM_AC,p.H1_H0*(1-p.SYM_AC),(1 - H1C.rem)*p.H1_AC,(1 - H1C.rem)*(1-p.H1_AC),p.H1C_SCS,p.H1C_ED,p.H1C_Hosp, 0, p.H1C_D[t-1])
    m.p.it[,M_it == "H1NC"]  <- c(p.H1_H0*p.SYM_AC,p.H1_H0*(1-p.SYM_AC),(1 - H1NC.rem)*p.H1_AC,(1 - H1NC.rem)*(1-p.H1_AC),p.H1NC_SCS,p.H1NC_ED,p.H1NC_Hosp, 0, p.H1NC_D[t-1])
    m.p.it[,M_it == "SCS"] <- c(0, 0, 0, 0, 0, 0, 0,(1-SCS.rem), min(p.SCS_D[t-1]+p.H0C_D[t-1],1))
    m.p.it[,M_it == "ED"] <- c(0, 0, 0, 0, 0, 0, 0,(1-ED.rem), min(p.ED_D[t-1]+p.H0C_D[t-1],1))
    m.p.it[,M_it == "Hosp"] <- c(0, 0, 0, 0, 0, 0, 0,(1-Hosp.rem), min(p.Hosp_D[t-1]+p.H0C_D[t-1],1))
    m.p.it[,M_it == "SEVR"] <- c(0, 0, p.SEVR_H1*p.H1_AC, p.SEVR_H1*(1-p.H1_AC), 0, 0, 0, 1-SEVR.rem, p.H0C_D[t-1])
  ...
  Costs <- function (M_it) {
    ...
    c.it <- 0                                  # by default the cost for everyone is zero 
    c.it[M_it == "H0C"]  <- c.SYM
    c.it[M_it == "H0NC"]  <-  c.SYM + c.AU + c.prodAU
    c.it[M_it == "H1C"]  <- c.ICS_SABA + c.HCP + c.timeHCP
    c.it[M_it == "H1NC"]  <- c.ICS_SABA + c.HCP + c.timeHCP + c.AU + c.prodAU
    c.it[M_it == "SCS"]  <- c.SCS + c.prodSCS
    c.it[M_it == "ED"]  <-  c.ED + c.prodED
  ...
  m.C[, 1:2] <- Costs(m.M[, 1:2])  # estimate costs per individual for the initial health state
  m.E[, 1:2] <- Effs (m.M[, 1:2])  # estimate QALYs per individual for the initial health state
...
```{r, PSA CEA plotting}
#Importing data
res <- read_csv("C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\PSAResults_ALL19.csv")
res$ID <- rep(1:1000,3)
...
sym_prim <- CEA_plot(data = resw, x = resw$deltaSP_QALYs, y = resw$deltaSP_Costs, xlab="Incremental QALY", ylab = "Incremental Cost")
sym_noTrt <- CEA_plot(data = resw, x = resw$deltaSN_QALYs, y = resw$deltaSN_Costs, xlab="Incremental QALY", ylab = "Incremental Cost")
noTrt_prim <- CEA_plot(data = resw, x = resw$deltaNP_QALYs, y = resw$deltaNP_Costs, xlab="Incremental QALY", ylab = "Incremental Cost")

theme_set(theme_pubr())
z <- ggarrange(sym_prim,sym_noTrt,noTrt_prim,
               ncol=1,
               nrow=3,
               labels =c('A','B','C'))

ggsave("CEA_plotv3_19.jpeg", plot=z, 
...
#CEAC comparing bud-form to no OTc
 SP.prop <- SP[[2]]
 SP.prop$trt  <- "Budesonide-formoterol"
 PS.prop <- NP [[2]]
 PS.prop$prop <- 1-SP.prop$prop
 PS.prop$trt <- "Inhaled epinephrine"
 
 SP_prop <- rbind(SP.prop,PS.prop) 

#CEAC comparing bud-form to no OTC
 SN.prop <- SN[[2]]
 SN.prop$trt  <- "Budesonide-formoterol"
 
 NS.prop <- SP[[2]]
 NS.prop$prop <- 1-SN.prop$prop
 NS.prop$trt  <- "No OTC inhaler"
 
 SN_prop <- rbind(SN.prop, NS.prop)

 #CEAC comparing no OTC to inhaled epinephrine
 NP.prop <- NP[[2]]
 NP.prop$trt  <- "No OTC inhaler"
 
 PN.prop <- NP[[2]]
 PN.prop$trt  <- "Inhaled epinephrine"
 PN.prop$prop <- 1-NP.prop$prop
 

 
 NP_prop <- rbind(NP.prop, PN.prop)
...
a<-CEAC_plot(SP_prop) + scale_colour_manual(values=c("Budesonide-formoterol"= "red", "Inhaled epinephrine"="blue"))
b<-CEAC_plot(SN_prop) + scale_colour_manual(values=c("Budesonide-formoterol"= "red","No OTC inhaler"="orange"))
c<-CEAC_plot(NP_prop) + scale_colour_manual(values=c("No OTC inhaler"="orange","Inhaled epinephrine"="blue"))
...
ggsave("CEAC_plot2_19.png", plot=y,
       path="C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Manuscript/Figures",
       width= 7,
       height= 11,
       dpi = 1000)
...
tornado_plot <- function(df, wtp=100000){
  ...
  dsa_names<-c("Annual probability - Transitioning from no HCP to HCP (0%,20%)","Annual probability - Non-adherence to HCP (+/-20%)",
             "Annual rate of severe exacerbation - No OTC inhaler/IE (+/-20%)","Annual rate of severe exacerbation - Bud-form (+/-20%)", 
             "Annual rate of severe exacerbation - HCP (+/-20%)",
             "Cost - HCP management (+/-20%)","Cost - Asthma not well-controlled day (+/-20%)","Cost of severe exacerbation - SCS use (+/-20%)",
             "Cost of severe exacerbation - ED visit + SCS use (+/-20%)","Cost of severe exacerbation - Hospitalization (+/-20%)",
             "Inhalations per day - IE (+/-20%)","Inhalations per day - Bud-form (+/-20%)","Inhalations per day - ICS (+/-20%)","Inhalations per day - SABA (+/-20%)",
             "Disutility - Asthma not well-controlled day (+/-20%)","Disutility of severe exacerbation - SCS use (+/-20%)",
             "Disutility of severe exacerbation - ED visit + SCS use (+/-20%)",
             "Disutility of severe exacerbation - Hospitalization (+/-20%)",
             "Utility - Non-exacerbation (+/-20%)",
             "Non-adherence - Bud-form (0%,20%)")
...
```
```{r, Scenario analysis}
res_dc0 <- read_csv("C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_dc0.csv")
res_dc6 <- read_csv("C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_dc6.csv")
res_th10 <- read_csv("C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_th10.csv")
res_th30 <- read_csv("C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_th30.csv")
res_public <- read_csv("C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\scenario19_public.csv")
...
write_csv(table_markov_spread,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\ScenResults19.csv")
...
###### Run the simulation ######
p = Sys.time()
sim_no_trt  <- MicroSim(v.M_1, n.i, n.t, v.n, d.c, d.e, Trt=F, seed=1996) # run for no treatment
comp.time = Sys.time() - p
comp.time

p = Sys.time()
sim_PRIM  <- MicroSim(v.M_1, n.i, n.t, v.n, d.c, d.e, Trt="primatene", seed=1996) # run for no treatment
comp.time = Sys.time() - p
comp.time

p = Sys.time()
sim_SYMB  <- MicroSim(v.M_1, n.i, n.t, v.n, d.c, d.e, Trt="symbicort", seed=1996) # run for no treatment
comp.time = Sys.time() - p
comp.time

simStore<-function(df){
  
  d<-cbind(df$tc,df$te)
  colnames(d)<-c("tc","te")
  return(d)
}
simStore2<-function(df){
  ad<-sum(sum(df[["TS"]]=="SCS->D"),sum(df[["TS"]]=="ED->D"),sum(df[["TS"]]=="Hosp->D"))
  
  v <- c(df$tc_hat, df$te_hat, ad)
  
}

#Matrix calculations for no treatment
nt_agg <- as.data.frame(rowSums(sim_no_trt[["m.M"]] =="H0C" | sim_no_trt[["m.M"]] =="H1C")*7)
nt_agg$exac <- rowSums(sim_no_trt[["m.M"]] =="SCS" | sim_no_trt[["m.M"]] =="ED" | sim_no_trt[["m.M"]] =="Hosp")
  colnames(nt_agg) <- c("control","exac")
  apply(nt_agg,2,mean)
  s.d<-apply(nt_agg,2,sd)
  s.d/sqrt(n.i)

nt1<-simStore(sim_no_trt)
nt2<-simStore2(sim_no_trt)
write.csv(nt1,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\nt1_19.csv")
write.csv(nt2,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\nt2_19.csv")
write.csv(nt_agg, "C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\nt_EC_19.csv")

#Matrix calculations
prim_agg <- as.data.frame(rowSums(sim_PRIM[["m.M"]] =="H0C" | sim_PRIM[["m.M"]] =="H1C")*7)
prim_agg$exac <- rowSums(sim_PRIM[["m.M"]] =="SCS" | sim_PRIM[["m.M"]] =="ED" | sim_PRIM[["m.M"]] =="Hosp")
  colnames(prim_agg) <- c("control","exac")
  apply(prim_agg,2,mean)
  s.d<-apply(prim_agg,2,sd)
  s.d/sqrt(n.i)

p1<-simStore(sim_PRIM)
p2<-simStore2(sim_PRIM)
write.csv(p1,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\p1_19.csv")
write.csv(p2,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\p2_19.csv")
write.csv(prim_agg, "C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\prim_EC_19.csv")


#Matrix calculations
symb_agg <- as.data.frame(rowSums(sim_SYMB[["m.M"]] =="H0C" | sim_SYMB[["m.M"]] =="H1C")*7)
symb_agg$exac <- rowSums(sim_SYMB[["m.M"]] =="SCS" | sim_SYMB[["m.M"]] =="ED" | sim_SYMB[["m.M"]] =="Hosp")
  colnames(symb_agg) <- c("control","exac")
  apply(symb_agg,2,mean)
  s.d<-apply(symb_agg,2,sd)
  s.d/sqrt(n.i)
  
s1<-simStore(sim_SYMB)
s2<-simStore2(sim_SYMB)
write.csv(s1,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\s1_19.csv")
write.csv(s2,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\s2_19.csv")
write.csv(symb_agg, "C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\symb_EC_19.csv")
...
v.C  <- c(sim_no_trt$tc_hat, sim_PRIM$tc_hat, sim_SYMB$tc_hat) 
se.C <- c(sd(sim_no_trt$tc), sd(sim_PRIM$tc),sd(sim_SYMB$tc)) / sqrt(n.i)

# store the mean QALYs (and the MCSE) of each strategy in a new variable v.E (vector health outcomes)
v.E  <- c(sim_no_trt$te_hat, sim_PRIM$te_hat, sim_SYMB$te_hat) 
se.E <- c(sd(sim_no_trt$te), sd(sim_PRIM$te),sd(sim_SYMB$te)) / sqrt(n.i)

v.ad <- c(sum(sum(sim_no_trt[["TS"]]=="SCS->D"),sum(sim_no_trt[["TS"]]=="ED->D"),sum(sim_no_trt[["TS"]]=="Hosp->D")),
          sum(sum(sim_PRIM[["TS"]]=="SCS->D"),sum(sim_PRIM[["TS"]]=="ED->D"),sum(sim_PRIM[["TS"]]=="Hosp->D")),
          sum(sum(sim_SYMB[["TS"]]=="SCS->D"),sum(sim_SYMB[["TS"]]=="ED->D"),sum(sim_SYMB[["TS"]]=="Hosp->D"))
          )

v.ac <- c(sum(sim_no_trt[["tc"]]), sum(sim_PRIM[["tc"]]), sum(sim_SYMB[["tc"]])) 

#incremental 
delta.CN <- v.C[1] - v.C[2]                   # calculate incremental costs
delta.EN <- v.E[1] - v.E[2]                   # calculate incremental QALYs
delta.CS <- v.C[3] - v.C[2]                   # calculate incremental costs
delta.ES <- v.E[3] - v.E[2]                   # calculate incremental QALYs

se.delta.EN <- sd(sim_no_trt$te - sim_PRIM$te) / sqrt(n.i) # Monte Carlo squared error (MCSE) of incremental costs
se.delta.CN <- sd(sim_no_trt$tc - sim_PRIM$tc) / sqrt(n.i) # Monte Carlo squared error (MCSE) of incremental QALYs
se.delta.ES <- sd(sim_SYMB$te - sim_PRIM$te) / sqrt(n.i) # Monte Carlo squared error (MCSE) of incremental costs
se.delta.CS <- sd(sim_SYMB$tc - sim_PRIM$tc) / sqrt(n.i) # Monte Carlo squared error (MCSE) of incremental QALYs

ICER.N    <- delta.CN / delta.EN                 # calculate the ICER
ICER.S    <- delta.CS / delta.ES  
...
table_micro <- data.frame(
  ...
  c(round(v.E, 6),  ""),           # health outcomes per arm
  c(round(se.E, 6), ""),           # MCSE for health outcomes
  c(round(delta.CN,0),  "",  round(delta.CS,0),""),  # incremental costs
  c(round(se.delta.CN, 0),"" ,round(se.delta.CS, 0),""),  # MCSE for incremental costs
  c(round(delta.EN, 6),"" ,round(delta.ES, 6),""),  # incremental QALYs 
  c(round(se.delta.EN, 6),"" ,round(se.delta.ES, 6),""),  # MCSE for health outcomes (QALYs) gained
  c(round(ICER.N, 0),"",round(ICER.S, 0),""),   # ICER
  c(v.ad,""), #asthma deaths
  c(v.ac,"") #total costs
)
rownames(table_micro) <- c(v.Trt, "* are MCSE values")  # name the rows
colnames(table_micro) <- c("Costs", "*",  "QALYs", "*", "Incremental Costs", "*", "QALYs Gained", "*", "ICER", "Asthma death","total costs") # name the columns
table_micro  # print the table 

write.csv(table_micro,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\microResults.csv")


###### Cost-effectiveness analysis using imported dataset ######

#cost and qaly dataset
sim_no_trt <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/nt1_19.csv")
sim_PRIM <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/p1_19.csv")
sim_SYMB <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/s1_19.csv")

#Exac and control dataset
sim_no_trt_EC <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/nt_EC_19.csv")
sim_PRIM_EC <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/prim_EC_19.csv")
sim_SYMB_EC <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/symb_EC_19.csv")

#Asthma-Death dataset
sim_no_trt_D <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/nt2_19.csv")
sim_PRIM_D <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/p2_19.csv")
sim_SYMB_D <- read_csv("C:/Users/Joseph/Desktop/Grad studies MSc and Applications/AsthmaCEA2022/Preliminary results/s2_19.csv")


## microsim values
n.i <- 100000
trt <- c('noTrt', 'PRIM', 'SYMB')

## calculating mean 
m.C <- c(mean(sim_no_trt$tc), mean(sim_PRIM$tc),mean(sim_SYMB$tc)) #mean cost
m.E <- c(mean(sim_no_trt$te), mean(sim_PRIM$te),mean(sim_SYMB$te)) #mean qalys
m.CONT <- c(mean(sim_no_trt_EC$control), mean(sim_PRIM_EC$control),mean(sim_SYMB_EC$control)) #mean control days
m.EXAC <- c(mean(sim_no_trt_EC$exac), mean(sim_PRIM_EC$exac),mean(sim_SYMB_EC$exac)) #mean exac
m.AD <- c(sim_no_trt_D$x[3],sim_PRIM_D$x[3], sim_SYMB_D$x[3])/n.i #asthma-related deaths

## calculating SE
se.C <- c(sd(sim_no_trt$tc), sd(sim_PRIM$tc),sd(sim_SYMB$tc)) / sqrt(n.i) #SD cost
se.E <- c(sd(sim_no_trt$te), sd(sim_PRIM$te),sd(sim_SYMB$te)) / sqrt(n.i) #SD qalys
se.CONT <- c(sd(sim_no_trt_EC$control), sd(sim_PRIM_EC$control),sd(sim_SYMB_EC$control)) / sqrt(n.i) #SD control days
se.EXAC <- c(sd(sim_no_trt_EC$exac), sd(sim_PRIM_EC$exac),sd(sim_SYMB_EC$exac)) / sqrt(n.i) #SD exac
se.AD <- sqrt((m.AD*(1-m.AD)/n.i)) #SD asthma-related deaths

### Incremental SE
sd(sim_SYMB$tc - sim_PRIM$tc)/sqrt(n.i) #Symb vs prim cost
sd(sim_no_trt$tc - sim_PRIM$tc)/sqrt(n.i) #no trt vs prim cost

sd(sim_SYMB$te - sim_PRIM$te)/sqrt(n.i) #Symb vs prim eff
sd(sim_no_trt$te - sim_PRIM$te)/sqrt(n.i) #no trt vs prim eff

## total results for microsim
ext <- 52.5

totC_pop <- c(sum(sim_no_trt$tc),sum(sim_PRIM$tc),sum(sim_SYMB$tc))*ext # total costs
totE_pop<-c(sum(sim_no_trt_EC$exac),sum(sim_PRIM_EC$exac),sum(sim_SYMB_EC$exac))*ext # total exac
totAD_pop <- c(sim_no_trt_D$x[3],sim_PRIM_D$x[3], sim_SYMB_D$x[3])*ext # total deaths

table_micro$totC_pop[3] - table_micro$totC_pop[1] #total incremental cost for SYMB and PRIM
table_micro$totE_pop[3] - table_micro$totE_pop[1] #total incremental exac for SYMB and PRIM
table_micro$totAD_pop[3] - table_micro$totAD_pop[1] #total incremental exac for SYMB and PRIM
  
## micro table
table_micro <- data.frame(trt,m.C,se.C,m.E,se.E,m.CONT,se.CONT,m.EXAC,se.EXAC, m.AD, se.AD, totC_pop, totE_pop, totAD_pop) %>% arrange(m.C)
write.csv(table_micro,"C:\\Users\\Joseph\\Desktop\\Grad studies MSc and Applications\\AsthmaCEA2022\\Preliminary results\\microResults_v2.csv")
...
n.t   <- 4006           # time horizon, lifetime from 23yrs, 1 week cycle - 77 years and 1 week
...
sim_base_no_trt  <- Markov(params, v.M_1, n.t, v.n, d.c, d.e, res=T)
sim_base_PRIM    <- Markov(params, v.M_1, n.t, v.n, d.c, d.e,trt="primatene", res=T)
sim_base_SYMB    <- Markov(params, v.M_1, n.t, v.n, d.c, d.e,trt="symbicort", res=T)  
