...
## Severe Exacerbation proportion (SCS, ED, HOSP)
  ...
  p.SYM_SCS <- (1-exp(-0.11/52)) * prop.SYM_SEV[1]	# weekly rate of severe exacerbation with symbicort PRN
  p.SYM_ED <- (1-exp(-0.11/52)) * prop.SYM_SEV[2]
  p.SYM_Hosp <- (1-exp(-0.11/52)) * prop.SYM_SEV[3]
  
  p.H1_SCS <- (1-exp(-0.12/52)) * prop.H1_SEV[1]	# weekly rate of severe exacerbation with HCP management (low dose ICS + SABA)
...
## Transition probabilities #
  ...
  #H0
    ...
    pAnn.H0_D   <-  mortality$b_mortality   # probability of death from no HCP management (turn into vector)
...
# Arguments:  
  ...
  for (i in 2:(n.t + 1)) { 
    for (j in 1:n.t) {
      ...
      m.P <- matrix(c(1-min((p.H0_H1+p.H0_SCS+p.H0_ED+p.H0_Hosp+p.H0_D[j]),1), p.H0_H1, p.H0_SCS, p.H0_ED, p.H0_Hosp, p.H0_D[j], #H0 (no hcp mgt)
                      p.H1_H0, 1 - min((p.H1_H0+p.H1_SCS+p.H1_ED+p.H1_Hosp+p.H1_D[j]),1), p.H1_SCS, p.H1_ED, p.H1_Hosp, p.H1_D[j], #H1 (hcp mgt)
                      0, 1 - min((p.SCS_D_PRIM[j]+p.ED_D_PRIM[j]+p.Hosp_D_PRIM[j]+p.H0_D[j]),1), 0, 0, 0, min(p.SCS_D_PRIM[j]+p.H0_D[j],1), #SCS
                      0, 1 - min((p.SCS_D_PRIM[j]+p.ED_D_PRIM[j]+p.Hosp_D_PRIM[j]+p.H0_D[j]),1), 0, 0, 0, min(p.ED_D_PRIM[j]+p.H0_D[j],1), #ED
                      0, 1 - min((p.SCS_D_PRIM[j]+p.ED_D_PRIM[j]+p.Hosp_D_PRIM[j]+p.H0_D[j]),1), 0, 0, 0, min(p.Hosp_D_PRIM[j]+p.H0_D[j],1), # Hosp
                      0, 0, 0, 0, 0, 1), #D
  ...
  v.u <- c(u.H0 + ((1- p.H0_AC)*du.AU), #H0
           u.H1 + ((1- p.H1_AC)*du.AU), #H1
           u.SCS, #SCS
           u.ED, #ED
           u.Hosp, #HOSP
           0) #D
  ...
  for (i in 2:(n.t + 1)) { 
    for (j in 1:n.t) {
      ...
      # create the transition matrix for each model cycle
      m.P <- matrix(c(1-min((p.H0_H1+p.H0_SCS+p.H0_ED+p.H0_Hosp+p.H0_D[j]),1), p.H0_H1, p.SYM_SCS, p.SYM_ED, p.SYM_Hosp, p.H0_D[j], #H0 (no hcp mgt)
                      p.H1_H0, 1 - min((p.H1_H0+p.H1_SCS+p.H1_ED+p.H1_Hosp+p.H1_D[j]),1), p.H1_SCS, p.H1_ED, p.H1_Hosp, p.H1_D[j], #H1 (hcp mgt)
                      0, 1 - min((p.SCS_D[j]+p.ED_D[j]+p.Hosp_D[j]+p.H0_D[j]),1), 0, 0, 0, min(p.SCS_D[j]+p.H0_D[j],1), #SCS
                      0, 1 - min((p.SCS_D[j]+p.ED_D[j]+p.Hosp_D[j]+p.H0_D[j]),1), 0, 0, 0, min(p.ED_D[j]+p.H0_D[j],1), #ED
                      0, 1 - min((p.SCS_D[j]+p.ED_D[j]+p.Hosp_D[j]+p.H0_D[j]),1), 0, 0, 0, min(p.Hosp_D[j]+p.H0_D[j],1), # Hosp
                      0, 0, 0, 0, 0, 1), #D
                    nrow = n.s, ncol = n.s, byrow = T,
                    dimnames = list(v.n, v.n))
      print(m.P) # show transition matrix for each model cycle to make sure p.H0_D is working - can remove after debugging
  ...
  v.u <- c(u.H0 + ((1- p.SYM_AC)*du.AU), #H0
           u.H1 + ((1- p.H1_AC)*du.AU), #H1
           u.SCS, #SCS
           u.ED, #ED
           u.Hosp, #HOSP
...
v.C <- c(sim_markov_no_trt$tc_hat, sim_markov_PRIM$tc_hat) 
# store the mean QALYs of each strategy in a new variable E (vector effects)
v.E <- c(sim_markov_no_trt$te_hat, sim_markov_PRIM$te_hat)
