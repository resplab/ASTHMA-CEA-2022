...
### Model input
  ...
  v.M_1 <- c(1,0,0,0)                 # Initial health state distribution 
...
## Severe Exacerbation Rates
  r.H0_SEV <- r.PRIM_SEV <- 0.20                    	# annual rate of severe exacerbation with no OTC or primatene OTC prn
  r.SYM_SEV <- 0.11 	                                # annual rate of severe exacerbation with symbicort PRN
  r.H1_SEV <- 0.12 	                                  # annual rate of severe exacerbation with HCP management (low dose ICS + SABA)
  
## Transition probabilities 
  ##Annual
    pAnn.H0_H1   <- 0                                   # probability entering HCP from no HCP management (can't go to H1 without sev)
    pAnn.H0_SEV  <- (1-exp(-r.H0_SEV))                  # probability of severe exacerbation from no HCP management
    ...
    pAnn.H1_SEV  <- (1-exp(-r.H1_SEV))                  # probability of severe exacerbation from HCP management
    pAnn.H1_D   <- pAnn.H0_D                            # probability of death from HCP management (turn into vector)
    
    pAnn.SEV_H0 <- 0
    pAnn.SEV_H1 <- 1                                    # ***(probability of Severe exacerbation to HCP management is basically just 1 - whoever doesnt die)
    pAnn.SEV_D <-  0.0052                               # ****probability of asthma fatality from hospitalization* we need an overall probability/rate of death from severity
  
  ##WEEKLY (per cycle)****
    p.H0_H1   <- 0                                        # probability entering HCP from no HCP management (can't go to H1 without sev)
    p.H0_SEV  <- 1 - (1-pAnn.H0_SEV)^(1/52)               # probability of severe exacerbation from no HCP management
    p.H0_D   <-  1 - (1-pAnn.H0_D)^(1/52)                 # probability of death from no HCP management (turn into vector)
  
    p.H1_H0  <- 1 - (1-pAnn.H1_H0)^(1/52)                 # probability of non-adherence to HCP management
    p.H1_SEV  <- 1 - (1-pAnn.H1_SEV)^(1/52)               # probability of severe exacerbation from HCP management
    p.H1_D   <- p.H0_D                                    # probability of death from HCP management (turn into vector)
    
    p.SEV_H0 <- 0
    p.SEV_H1 <- 1 - (1-pAnn.SEV_H1)^(1/52)              #(probability of Severe exacerbation to HCP management is basically just 1 - whoever doesnt die)
    p.SEV_D <-  1 - (1-pAnn.SEV_D)^(1/52)               # probability of asthma fatality from hospitalization* we need an overall probability/rate of death from severity
...
## Indirect cost
  ...
  c.prodAU   <- 2 # cost of productivity loss due to asthma not controlled days
...
texac <- m.TR[,3] %*% t(prop.H0_SEV)

tc <-( m.TR %*% v.c) + ( texac %*% v.c_exac)  # calculate the costs, vc should be 0 for severe exac state
te <- m.TR %*% v.u + ( texac %*% v.u_exac)  # calculate the QALYs, vu should be 0 for severe exac state
