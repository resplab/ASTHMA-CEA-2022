...
for (i in 2:(n.t + 1)) { 
  ...
  SEVR.rem <-  (p.SEVR_H1*p.H1_AC + p.SEVR_H1*(1-p.H1_AC))

    
  # create the transition matrix for each model cycle
    m.P <- matrix(c(
                    (1-H0C.rem)*p.H0_AC,(1-H0C.rem)*(1-p.H0_AC), 0,0, p.H0C_SCS, p.H0C_ED, p.H0C_Hosp, 0, p.H0C_D[i-1], #H0C
                    (1-H0NC.rem)*p.H0_AC,(1-H0NC.rem)*(1-p.H0_AC), 0,0, p.H0NC_SCS, p.H0NC_ED, p.H0NC_Hosp, 0, p.H0NC_D[i-1], #H0NC 
                    ...
                    0, 0, p.SEVR_H1*p.H1_AC, p.SEVR_H1*(1-p.H1_AC), 0, 0, 0, 1-SEVR.rem, 0, #SEVR
                    0, 0, 0, 0, 0, 0, 0, 0, 1), #D
...
SEVR.rem <-  (p.SEVR_H1*p.H1_AC + p.SEVR_H1*(1-p.H1_AC))
  ...
  # create the transition matrix for each model cycle
    m.P <- matrix(c(
                    (1-H0C.rem)*p.SYM_AC,(1-H0C.rem)*(1-p.SYM_AC), 0,0, p.SYMC_SCS, p.SYMC_ED, p.SYMC_Hosp, 0, p.H0C_D[i-1], #H0C
                    (1-H0NC.rem)*p.SYM_AC,(1-H0NC.rem)*(1-p.SYM_AC), 0,0, p.SYMNC_SCS, p.SYMNC_ED, p.SYMNC_Hosp, 0, p.H0NC_D[i-1], #H0NC 
...
H0C.rem <- min((0+0+p.H0C_SCS+p.H0C_ED+p.H0C_Hosp+0+p.H0C_D[i-1]),1)
H0NC.rem <- min((0+0+p.H0NC_SCS+p.H0NC_ED+p.H0NC_Hosp+0+p.H0NC_D[i-1]),1)
...
#outcomes
  ...
  cont <- sum(m.TR_temp$H0C + m.TR_temp$H1C)
  UC <- sum(m.TR_temp$H0NC + m.TR_temp$H1NC + m.TR_temp$SEVR)
