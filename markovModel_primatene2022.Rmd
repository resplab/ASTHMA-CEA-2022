...
## Transition probabilities (per cycle)
  ...
  p.H0_SEV  <- 0.00384                # probability of severe exacerbation from no HCP management
  p.H0_D   <-  0.00097                # probability of death from no HCP management (turn into vector)

  p.H1_H0  <- 0.0296                 # probability of non-adherence to HCP management
  p.H1_SEV  <- 0.002                 # probability of severe exacerbation from HCP management
  p.H1_D   <- p.H0_D                 # probability of death from HCP management (turn into vector)
  
  p.SEV_H0 <- 0
  p.SEV_H1 <- 1                        #(probability of Severe exacerbation to HCP management is basically just 1 - whoever doesnt die)
  p.SEV_D <-  0.0000613               # probability of asthma fatality from hospitalization* we need an overall probability/rate of death from severity
...
## Severe Exacerbation rates (weekly probability)
  ...
  p.nOTC_SEV <- 1 - exp(-r.nOTC_SEV/52)	 # weekly probability of severe exacerbation with no OTC or primatene OTC prn (Base)
...
exac_sev <- c(0.78,0.13,0.09)
texac <- m.TR[,3] %*% exac_sev
v.c.exac <- c(163, 744, 12398)


tc <-( m.TR %*% v.c) +( texac %*% v.c.exac)  # calculate the costs, vc should be 0 for severe exac state
