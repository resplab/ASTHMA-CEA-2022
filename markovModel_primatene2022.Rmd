...
table_markov$Strategy <- c("No treatment","Primatine","Symbicort")
table_markov <- select(table_markov, Strategy, everything())
colnames(table_markov) <- c("Strategy","Costs","Effectiveness")

# calculate ICER
table_markov <- table_markov %>%
                    arrange(desc(Effectiveness)) %>% # no treatment is dominated by symbicort 
                    mutate(delta_Costs=c(Costs[1]-Costs[3],0,0),  # calculate incremental costs
                           delta_Effectiveness=c(Effectiveness[1]-Effectiveness[3],0,0), # calculate incremental QALYs
                           delta_ICER=delta_Costs/delta_Effectiveness) # calculate the ICER
